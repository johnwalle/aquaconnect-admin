...
export default function UsersPage() {
  ...
  const stats = [
    ['Total Users', MOCK_USERS.length, 'registered customers'],
    ['Active', MOCK_USERS.filter(u => u.status === 'ACTIVE').length, 'currently active'],
    ['Flagged', MOCK_USERS.filter(u => u.paymentFlag !== 'NONE').length, 'payment issues'],
    ['No Meter', MOCK_USERS.filter(u => !u.meter).length, 'unassigned meters'],
  ];
    ...
    <div className="text-[#e8f4f0]">
      ...
      <div className="bg-[#05141f] border border-[rgba(29,158,117,0.08)] rounded-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[rgba(29,158,117,0.08)]">
          <div>
            <h2 className="font-syne font-bold text-sm tracking-tight">All Users</h2>
        ...
        <div className="flex items-center gap-3 px-6 py-3 border-b border-[rgba(29,158,117,0.06)] flex-wrap">
          ...
          >
            {FLAGS.map((f) => (
              <option key={f} value={f}>{f || 'All Payment Flags'}</option>
            ))}
          </select>
          {(search || filterStatus || filterFlag) && (
            <button
              onClick={() => { setSearch(''); setFilterStatus(''); setFilterFlag(''); setPage(1); }}
